"""
--------------------------
File Name:  Visualization_multicase_juwels.py
...
ntimestep = 4 * 12  # statistic end time
nstart = 4 * 1  # statistic start time

# ---------------------------------
# Work Direction and file path.
# ---------------------------------
# work_dir = r'/home/zhang/practise/remap_torus'  # Work directory.
work_dir = r'/p/scratch/cslts/zhang23/JUWELS_RiverLike/REMAP'
# work_dir = r'/home/zhang/cslts_zhang23/JUWELS_RiverLike/REMAP'
os.chdir(work_dir)

folder_output = 'python_visualization_multicase_v02'  # folder for the output figures.
if not os.path.exists(folder_output):
    os.mkdir(folder_output)

# Set file path (Since each case have different number of rivers, we do it
# case by case.
...
def Draw_figures(file_insta, file_land, file_rad, file_prof, file_tseries, n_river, ysize, folder_output):
    """
    :param file_insta: File_insta
    :param file_land:  File_land
    :param file_rad:    File_rad
    :param file_prof:   File_Prof
    :param file_tseries:    File_Tseries
    :param n_river: The number of rivers in the domain.
    :return: Visualization the ICON-lem OUTPUT (i.e. draw the figures).
    """
    # -------------------
    # Opena and read data
    # -------------------
    ...
    # ----------------------------------------
    tsfc = nc_land['t_g'][nstart:ntimestep, :, :]  # Weighted Surface Temperature.
    wth = nc_prof['wth'][nstart:ntimestep, :]  # Resolved Potential Temperature Flux.
    shfl_s = nc_land['shfl_s'][nstart:ntimestep, :, :]  # Surface Sensible Head Flux
    rho = nc_insta['rho'][nstart:ntimestep, nlevels - 1, :, :]  # Air Density at the lowest layer. (4:48, -1, :, :)

    shfl_s_avg = np.mean(np.mean(shfl_s, -1), -1)  # Domain-average.

    pbl = z_mc[np.argmin(wth[43, :])]  # Let's assume the time 6h.

    # convective velocity scale.
    w_star = ((g / tsfc[43, :, :]) * pbl * (-shfl_s_avg[43] / (rho[43, :, :] * cp))) ** (1 / 3)
    w_star = np.mean(np.array(w_star))

    # ---------
    # Calculate layer of the convective boundary layers
    # ---------
    layer_n = 64 - abs(z_mc - pbl).argmin()
    ''' Or using int(pbl/50);   int(pbl/62.625)'''
    layer_n12 = int(layer_n * 1.2)

    # --------------------
    # visualization
    # --------------------
    x = np.arange(ysize / n_river)

    # result_w = np.flipud(w_phase / w_star)[0:32, :]
    result_w = np.flipud(w_phase / w_star)[0:layer_n12, :]
    ...
    plt.xticks(ticks=[min(x), 0.5 * min(x + 0.5 * max(x) + min(x)), 0.5 * (max(x) + min(x)),
                      0.5 * max(x + 0.5 * (max(x) + min(x))), max(x)],
               labels=[0, '', 0.5, '', 1])
    plt.xlabel("x/\u03BB", fontsize=14)
    # plt.yticks(ticks=[0, 8, 16, 24, 31], labels=[0, 0.3, 0.6, 0.9, 1.2])
    plt.yticks(ticks=[0, int(layer_n12 / 4), int(layer_n12 / 2), int(layer_n12 / 4 * 3), layer_n12],
               labels=[0, 0.3, 0.6, 0.9, 1.2])
    plt.ylabel("z/z_i", fontsize=14)
    plt.savefig(os.path.join(folder_output, "w_phase.png"))
    plt.close()

    # result_v = np.flipud(v_phase / w_star)[0:32, :]
    result_v = np.flipud(v_phase / w_star)[0:layer_n12, :]
...
Draw_figures(file_insta, file_land, file_rad, file_prof, file_tseries, n_river=1, ysize=192, folder_output=folder_output)
Draw_figures(file_insta, file_land, file_rad, file_prof, file_tseries, n_river=2, ysize=192, folder_output=folder_output)
